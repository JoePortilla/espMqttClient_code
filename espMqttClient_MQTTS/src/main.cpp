/**
 * \file espMqttClient_MQTTS main.cpp
 * \brief Publicar un contador en broker MQTTS mediante la biblioteca espMqttClient
...
const uint16_t intervaloPub = 4000;            // Intervalo de tiempo en el que se publican los datos [ms].
uint32_t tActual = 0;                          // Almacena el tiempo actual de ejecución.
uint32_t tPrevioPub = 0;                       // Almacena el ultimo tiempo en que el dato fue publicado.
...
void loop()
{
  ...
  // Envío periodico de contador
  if (tActual - tPrevioPub >= intervaloPub)
  {
    // Aumentar contador
    contador++;
    // Publicar en el topico TOPIC_TEST, con un QOS=0 y RETENCION=false, la variable contador.
    mqttClient.publish(TOPIC_TEST, 0, false, String(contador).c_str());
    Serial.printf("Publicando 'Contador=%d' en tópico [%s] (QoS 0).\n", contador, TOPIC_TEST);
